...
export default function LessonsPage() {
  ...
  const roleParam = useMemo(() => {
    const role = user?.role ?? ROLE_PARAM_FALLBACK;
  ...
  const filteredLessons = useMemo(() => {
    ...
    return lessons.filter((lesson) => {
      ...
      const matchesEnhanced = showEnhancedOnly ? enhanced : true;
      ...
      return matchesSearch && matchesFormat && matchesEnhanced && matchesTags && matchesDuration;
    });
  }, [lessons, searchTerm, formatFilter, showEnhancedOnly, selectedTags, durationFilter]);
  ...
  useEffect(() => {
    ...
    async function fetchLessons() {
      try {
        ...
        const response = await fetch(`/api/lessons?role=${roleParam}`);
      ...
      <main className="min-h-[calc(100vh-130px)] bg-purplebg text-textdark">
        <div className="mx-auto w-full max-w-6xl space-y-8 px-6 py-[30px]">
          <header className="space-y-2">
            <h1 className="mb-[30px] pt-[45px] text-left text-3xl font-semibold text-primary">Lessons</h1>
          ...
          {loading ? (
            ...
            <p className="text-sm text-textdark/70">No lessons available yet.</p>
          ) : (
            <div className="grid gap-6 md:grid-cols-3">
              ...
              <aside className="flex flex-col gap-6 rounded-lg border border-[#D9D9D9] bg-white p-6 shadow-sm">
                ...
                <div className="flex items-center justify-between gap-3">
                  <label htmlFor="lesson-enhanced" className="text-sm text-textdark/80">
                    Show enhanced-only lessons
                  </label>
                  <input
                    id="lesson-enhanced"
                    type="checkbox"
                    checked={showEnhancedOnly}
                    onChange={(event) => setShowEnhancedOnly(event.target.checked)}
                    className="h-4 w-4"
                  />
                </div>
